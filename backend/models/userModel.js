...
userSchema.statics.signup = async function (fullname, email, password) {
  ...
  if (!fullname) {
    throw Error("input fullname");
  }
  if (!email) {
    throw Error("input email");
  }
  if (!password) {
  ...
  if (!validator.isStrongPassword(password)) {
    throw Error(
      "Password is not strong enough (should be 8 and above characters long, include uppercase letters, numbers and symbols)"
    );
  }
